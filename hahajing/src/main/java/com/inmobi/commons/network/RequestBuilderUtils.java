...
public class RequestBuilderUtils {
    ...
    public static void fillLocationMap(Map<String, Object> map) {
        if (map != null) {
            ...
            try {
                visbleWifiInfoBssId = IceDataCollector.getVisbleWifiInfoBssId(InternalSDKUtil.getContext());
            ...
            try {
                visbleWifiInfoBssId = CellUtil.getVisibleCellTowerIds(InternalSDKUtil.getContext());
                if (!(visbleWifiInfoBssId == null || visbleWifiInfoBssId.size() == 0)) {
                    map.put("v-sid", InternalSDKUtil.convertListToDelimitedString(visbleWifiInfoBssId, ","));
                }
            } catch (Throwable e22) {
                Log.internal(InternalSDKUtil.LOGGING_TAG, "Couldn't get cell tower info in Request Builder", e22);
            }
            try {
                currentLocationStr = CellUtil.getCurrentCellTower(InternalSDKUtil.getContext()).id;
                if (currentLocationStr != null && !"".equals(currentLocationStr)) {
                    map.put("c-sid", currentLocationStr);
                }
            } catch (Throwable e222) {
                Log.internal(InternalSDKUtil.LOGGING_TAG, "Couldn't get cell tower info in Request Builder", e222);
            }
        }
    }
    ...
    public static void fillIdentityMap(Map<String, Object> map, Map<String, Boolean> map2, boolean z) {
        if (map != null) {
            ...
            if (UID.getInstance().isLimitAdTrackingEnabled()) {
                map.put(AdTrackerConstants.KEY_LIMIT_ADTRACKING, Integer.valueOf(1));
            } else {
                map.put(AdTrackerConstants.KEY_LIMIT_ADTRACKING, Integer.valueOf(0));
            }
            if (z) {
                Map mapForEncryption = UID.getInstance().getMapForEncryption(map2);
            ...
            map.put("ts", Long.valueOf(instance.getTimeInMillis()));
            map.put("tz", Integer.valueOf(instance.get(16) + instance.get(15)));
