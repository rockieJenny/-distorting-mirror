...
public final class qo {
    ...
    public int rI() throws IOException {
        return (((rN() & 255) | ((rN() & 255) << 8)) | ((rN() & 255) << 16)) | ((rN() & 255) << 24);
    }
    ...
    public String readString() throws IOException {
        int rG = rG();
        if (rG > this.ayN - this.ayP || rG <= 0) {
            return new String(gV(rG), HttpRequest.CHARSET_UTF8);
        }
        String str = new String(this.buffer, this.ayP, rG, HttpRequest.CHARSET_UTF8);
        this.ayP = rG + this.ayP;
        return str;
    }
