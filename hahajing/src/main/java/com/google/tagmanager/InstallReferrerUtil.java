...
class InstallReferrerUtil {
    ...
    static void saveInstallReferrer(Context context, String referrer) {
        SharedPreferencesUtil.saveAsync(context, PREF_NAME_INSTALL_REFERRER, AdTrackerConstants.REFERRER, referrer);
        ...
        if (installReferrer == null) {
            synchronized (InstallReferrerUtil.class) {
                if (installReferrer == null) {
                    ...
                    if (settings != null) {
                        installReferrer = settings.getString(AdTrackerConstants.REFERRER, "");
                    } else {
                        installReferrer = "";
                    }
                }
