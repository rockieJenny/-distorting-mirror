...
class DefaultCreateReportSpiCall extends AbstractSpiCall implements CreateReportSpiCall {
    ...
    public boolean invoke(CreateReportRequest requestData) {
        ...
        Fabric.getLogger().d("Fabric", "Create report request ID: " + httpRequest.header(AbstractSpiCall.HEADER_REQUEST_ID));
    ...
    private HttpRequest applyHeadersTo(HttpRequest request, CreateReportRequest requestData) {
        request = request.header(AbstractSpiCall.HEADER_API_KEY, requestData.apiKey).header(AbstractSpiCall.HEADER_CLIENT_TYPE, AbstractSpiCall.ANDROID_CLIENT_TYPE).header(AbstractSpiCall.HEADER_CLIENT_VERSION, Crashlytics.getInstance().getVersion());
