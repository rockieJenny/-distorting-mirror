...
public class HListView extends AbsHListView {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (heightMode == 0) {
            ...
            heightSize |= ViewCompat.MEASURED_STATE_MASK & childState;
        }
        if (widthMode == 0) {
    ...
    private boolean commonKey(int keyCode, int count, KeyEvent event) {
        ...
        if (action != 1) {
            ...
            switch (keyCode) {
                ...
                case 20:
                    if (event.hasNoModifiers()) {
                        handled = handleHorizontalFocusWithinListItem(TransportMediator.KEYCODE_MEDIA_RECORD);
                        break;
                    }
                    break;
                case 21:
                ...
                case 22:
                    if (!event.hasNoModifiers()) {
                        if (event.hasModifiers(2)) {
                            handled = resurrectSelectionIfNeeded() || fullScroll(TransportMediator.KEYCODE_MEDIA_RECORD);
                            break;
                        }
                    }
                    handled = resurrectSelectionIfNeeded();
                    if (!handled) {
                        ...
                        while (true) {
                            count = count2 - 1;
                            if (count2 > 0 && arrowScroll(TransportMediator.KEYCODE_MEDIA_RECORD)) {
                                handled = true;
                                count2 = count;
                            }
                        }
                ...
                case 93:
                    ...
                    handled = resurrectSelectionIfNeeded() || pageScroll(TransportMediator.KEYCODE_MEDIA_RECORD);
                    break;
                    break;
                case 122:
                    if (event.hasNoModifiers()) {
    ...
    boolean fullScroll(int direction) {
        ...
        } else if (direction == TransportMediator.KEYCODE_MEDIA_RECORD && this.mSelectedPosition < this.mItemCount - 1) {
    ...
    private boolean handleHorizontalFocusWithinListItem(int direction) {
        if (direction == 33 || direction == TransportMediator.KEYCODE_MEDIA_RECORD) {
