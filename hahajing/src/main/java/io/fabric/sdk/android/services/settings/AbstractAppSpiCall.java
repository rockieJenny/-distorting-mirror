...
abstract class AbstractAppSpiCall extends AbstractSpiCall implements AppSpiCall {
    ...
    public boolean invoke(AppRequestData requestData) {
        ...
        Fabric.getLogger().d("Fabric", (HttpRequest.METHOD_POST.equals(httpRequest.method()) ? "Create" : "Update") + " app request ID: " + httpRequest.header(AbstractSpiCall.HEADER_REQUEST_ID));
    ...
    private HttpRequest applyMultipartDataTo(HttpRequest request, AppRequestData requestData) {
        ...
        if (requestData.sdkKits != null) {
            for (Kit kit : requestData.sdkKits) {
                request.part(sdkKitParamName(kit), kit.getVersion() == null ? "" : kit.getVersion());
            }
        }
        return request;
    }
    ...
    String sdkKitParamName(Kit kit) {
        return "app[build][libraries][" + (kit.getIdentifier() == null ? "" : kit.getIdentifier()) + "]";
    }
}
