...
class DefaultSettingsJsonTransform implements SettingsJsonTransform {
    ...
    public JSONObject sanitizeTraceInfo(JSONObject json) throws JSONException {
        JSONObject sanitized = new JSONObject(json.toString());
        sanitized.getJSONObject(SettingsJsonConstants.FEATURES_KEY).remove(SettingsJsonConstants.FEATURES_COLLECT_ANALYTICS_KEY);
        sanitized.remove(SettingsJsonConstants.ANALYTICS_KEY);
        return sanitized;
    }
    ...
    private AppIconSettingsData buildIconDataFrom(JSONObject iconJson) throws JSONException {
        return new AppIconSettingsData(iconJson.getString(SettingsJsonConstants.ICON_HASH_KEY), iconJson.getInt("width"), iconJson.getInt("height"));
    }
    ...
    private AnalyticsSettingsData buildAnalyticsSessionDataFrom(JSONObject json) {
        return new AnalyticsSettingsData(json.optString("url", SettingsJsonConstants.ANALYTICS_URL_DEFAULT), json.optInt(SettingsJsonConstants.ANALYTICS_FLUSH_INTERVAL_SECS_KEY, SettingsJsonConstants.ANALYTICS_FLUSH_INTERVAL_SECS_DEFAULT), json.optInt(SettingsJsonConstants.ANALYTICS_MAX_BYTE_SIZE_PER_FILE_KEY, 8000), json.optInt(SettingsJsonConstants.ANALYTICS_MAX_FILE_COUNT_PER_SEND_KEY, 1), json.optInt(SettingsJsonConstants.ANALYTICS_MAX_PENDING_SEND_FILE_COUNT_KEY, 100));
    }
