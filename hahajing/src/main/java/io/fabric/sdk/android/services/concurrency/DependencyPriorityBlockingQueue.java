...
public class DependencyPriorityBlockingQueue<E> extends PriorityBlockingQueue<E> implements OnCompletionListener<E> {
    ...
    E performOperation(int operation, Long time, TimeUnit unit) throws InterruptedException {
        switch (operation) {
            ...
            case 2:
                return super.poll();
            case 3:
                return super.poll(time.longValue(), unit);
            default:
                return null;
        }
    }
    ...
    E get(int operation, Long time, TimeUnit unit) throws InterruptedException {
        ...
        while (true) {
            ...
            if (result instanceof Task) {
                ((Task) result).addCompletionListener(this);
            }
    ...
    boolean canProcess(E result) {
        return !(result instanceof Task) || ((Task) result).canProcess();
    }
